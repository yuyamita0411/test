...
class CustomizerBuilder extends Builder {
    public function textfield($setting_id, $option)
    {
        ...
        $sanitize_callback = $sanitize ? 'wp_filter_nohtml_kses' : [$sanitizer, 'dangerously_skip_sanitize'];
        $default_setting = [
          'type'              => $updateMethod,
        ...
        $this->attr->add_control($setting_id, array(
          'settings' => $setting_id,
          'label' => $option['label'],
    ...
    public function textarea($setting_id, $option)
    {
        ...
        $updateMethod = $option['updateMethod'];
        $sanitize = isset($option['sanitize']) ? $option['sanitize'] : true;
        $sanitize_callback = $sanitize ? 'wp_kses_post' : [$sanitizer, 'dangerously_skip_sanitize'];
        $default_setting = [
          'type'              => $updateMethod,
          'transport'         => $option['transport'],
          'sanitize_callback' => $sanitize_callback,
    ...
    public function checkbox($setting_id, $option)
    {
        ...
        $default_setting = [
          'type'              => $updateMethod,
          'sanitize_callback' => [$sanitize, 'checkbox'],
          'transport'         => $option['transport'],
        ];
        $this->attr->add_setting(
            $setting_id,
    ...
    public function switch($setting_id, $option)
    {
        ...
        $default_setting = [
          'type'              => $updateMethod,
          'sanitize_callback' => [$sanitize, 'dangerously_skip_sanitize'],
          'transport'         => $option['transport'],
        ];
        if (isset($option['default'])) {
          $default_setting['default'] = $option['default'];
    ...
    public function radio($setting_id, $option)
    {
        ...
        $this->attr->add_setting(
            ...
            [
              'type'              => $updateMethod,
              'sanitize_callback' => [$sanitize, 'radio'],
              'transport'         => $option['transport'],
    ...
    public function select($setting_id, $option)
    {
        ...
        $this->attr->add_setting(
            ...
            [
              'type'              => $updateMethod,
              'transport'         => $option['transport'],
              'sanitize_callback' => [$sanitize, 'select'],
    ...
    public function color($setting_id, $option)
    {
        ...
        $this->attr->add_control(new WP_Customize_Color_Control($this->attr, $setting_id, array(
          'settings' => $setting_id,
          'label' => $option['label'],
          'section' => $option['section'],
          'description' => $option['description']
    ...
    public function file($setting_id, $option)
    {
        ...
        $default_setting = [
          'type'              => $updateMethod,
          'transport'         => $option['transport'],
          'sanitize_callback' => [$sanitizer, 'file'],
        ];
        if (isset($option['default'])) {
          $default_setting['default'] = $option['default'];
        }
    ...
    public function image($setting_id, $option)
    {
      ...
      $this->attr->add_control(new WP_Customize_Image_Control($this->attr, $setting_id, array(
        'settings' => $setting_id,
        'label' => $option['label'],
        'section' => $option['section'],
        'description' => $option['description']
    ...
    public function media($setting_id, $option)
    {
      ...
      $default_setting = [
        'type'              => $updateMethod,
        'transport'         => $option['transport'],
        'sanitize_callback' => [$sanitizer, 'dangerously_skip_sanitize'],
      ];
      if (isset($option['default'])) {
        $default_setting['default'] = $option['default'];
      }
      ...
      $this->attr->add_control(new WP_Customize_Media_Control($this->attr, $setting_id, array(
        'settings' => $setting_id,
        'label' => $option['label'],
        'section' => $option['section'],
        'description' => $option['description']
